...
- **Manage the project list with `muxy.projects` write verbs** instead of editing `projects.json` — `add(path)` registers an existing folder as a project, `rename(identifier, name)`, `setColor`/`setIcon`/`setLogo(identifier, value)` (pass `null` to clear), and `reorder(identifiers)` (the full list of project ids in the new order). All need `projects:write` and mutate Muxy's live project store, so the native sidebar updates immediately. `add` only accepts an existing directory; create the folder first via `muxy.files`/`muxy.exec` if needed. Subscribe to the `projects.changed` event (declare `events: ["projects.changed"]`, grant `projects:read`) to refetch via `muxy.projects.list()` after any change, whether made by your extension or Muxy itself. The home project cannot be renamed, recolored, re-iconed, or reordered. See [Permissions](https://muxy.app/docs/extensions/permissions).
